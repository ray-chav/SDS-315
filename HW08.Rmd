---
title: "Homework 08"
author: "Rachel Chavez - rcc3342 - {GitHub}"
date: "2025-04-09"
output:
...
The file covid.csv contains data on daily reported COVID-19 deaths for
Italy and Spain—two of the hardest-hit European countries—during the
first pandemic wave in February and March of 2020. The columns in this
data frame are: • date: the calendar date • country: Italy or Spain •
deaths: the number of reported COVID-19 deaths in that country on that
day • days_since_first_death: the number of days elapsed since the first
death in that country Your task is to fit two exponential growth models,
one for Italy and one for Spain, using days_since_first_death as the
time variable. Use the results of your model to characterize the growth
rate and doubling time of the daily death total in each country
...
model (log(Q)=log(K)+β⋅log(P)). We fit a linear
regression model with log(sales) as the response and log(price) as the
predictor. The estimated price elasticity (β̂) for our initial sample is -1.6186 , indicating
